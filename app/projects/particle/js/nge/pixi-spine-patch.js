PIXI.spine.Spine.prototype.replaceSpineRegion = function (
  attachment,
  sprite,
  region,
  assetKey
  ...
  const atlas = replacement
    ? nge.assets.getAtlasForTexture(replacement.name)
    : false;

  if (atlas && nge.wrap.cache.hasAtlas(atlas)) {
    sprite.region.texture = nge.wrap.cache.getAtlas(atlas).textures[
      replacement.name
    ];
...
PIXI.spine.Spine.prototype.setSpriteRegion = function (
  attachment,
  sprite,
  region
) {
  ...
  if (!region.size) {
    sprite.scale.x =
      (scaleFactor * attachment.scaleX * attachment.width) /
      region.originalWidth;
    sprite.scale.y =
      (scaleFactor * -attachment.scaleY * attachment.height) /
      region.originalHeight;
  } else {
    sprite.scale.x = (scaleFactor * region.size.width) / region.originalWidth;
    sprite.scale.y =
      (scaleFactor * -region.size.height) / region.originalHeight;
  }
};

PIXI.spine.core.TextureAtlas.prototype.defaultLoad =
  PIXI.spine.core.TextureAtlas.prototype.load;
  ...
  var iterateParser = function () {
    while (true) {
      ...
      if (line.length == 0) page = null;
        ...
        if (direction == "x") page.uWrap = PIXI.spine.core.TextureWrap.Repeat;
        else if (direction == "y")
        ...
        if (nge.wrap.cache.hasAtlas(atlas)) {
          texture = nge.wrap.cache.getAtlas(atlas).textures[
            region.name + ".png"
          ];
        } else {
...
PIXI.spine.core.TextureAtlas.prototype.load = function (
  atlasText,
  textureLoader,
  callback
) {
  if (textureLoader) this.defaultLoad(atlasText, textureLoader, callback);
  else this.packedlLoad(atlasText);
};

PIXI.spine.core.AnimationState.prototype.trackEntry = function (
  trackIndex,
  animation,
  loop,
  last
  ...
  if (!entry.timeScale) {
    ...
    Object.defineProperties(entry, {
      timeScale: {
        get: function () {
          return entry._timeScale * nge.statesManager.getTimeScale();
        },
        set: function (v) {
          entry._timeScale = v;
        },
